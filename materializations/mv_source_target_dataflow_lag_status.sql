...
create materialized view diag_mv_source_target_dataflow_lag_status as 
SELECT DISTINCT 
...
extract(epoch from mct.current_time) - source_info.time/1000 as current_source_lag_ms,
mcn.name as target_dataflow, 
to_timestamp(frontier_df.time/1000) as target_dataflow_last_updated_timestamp,
extract(epoch from mct.current_time) - frontier_df.time/1000 as current_dataflow_lag_ms,
CASE 
...
CROSS JOIN mv_current_time mct
JOIN 
...
extract(epoch from mct.current_time) - frontier_source.time/1000 as current_source_lag_ms,
mcn.name AS target_dataflow, 
to_timestamp(frontier_df.time/1000) as target_dataflow_last_updated_timestamp,
extract(epoch from mct.current_time) - frontier_df.time/1000 as current_dataflow_lag_ms,
